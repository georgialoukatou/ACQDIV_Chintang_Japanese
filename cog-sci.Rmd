...
author: "alejandrina cristia, georgia loukatou"
date: "4/27/2018, 16/07/2018 "
...
outfolder="~/Dropbox/acqdiv/" #location you want all figures and temp data to be stored in
...
# and like /scratch1/users/acristia/acqdiv//Japanese/words/full_part9/tags.txt_eval.ftp_rel.txt
data$folder=gsub("//","/",data$folder)
data$folder=gsub(folder,"",data$folder)
data$language=gsub("/.*", "", gsub("/scratch1/users/.*/acqdiv/","",data$folder))
...
data$subalgorithm=gsub("baselinesyll1","Syll=Word",data$subalgorithm)
data$subalgorithm=gsub("baselinesyll0","Utt=Word",data$subalgorithm)
...
data$wtypes_wtokens_ratio=data$words_types/data$words_tokens
...
All child-directed and child-overheard speech had been carefully transcribed in a transparent orthography (as well as the child's own speech, which was not analyzed here). After processing, the Chintang corpus contained  `r ch_w$corpus_nutts` utterances, with an average of `r round(ch_w$words_tokens/ch_w$corpus_nutts,2)` words, `r round(ch_w$syllables_tokens/ch_w$corpus_nutts,2)` syllables, and `r round(ch_w$phones_tokens/ch_w$corpus_nutts,2)` phones per utterance; and the Japanese one `r j_w$corpus_nutts` utterances, with an average of `r round(j_w$words_tokens/j_w$corpus_nutts,2)` words, `r round(j_w$syllables_tokens/j_w$corpus_nutts,2)` syllables, and `r round(j_w$phones_tokens/j_w$corpus_nutts,2)` phones per utterance. 

## General results section

```{r res-par1, echo=FALSE}
read.csv(paste0(outfolder,"/res-stat.txt"))->data
data$subalgorithm<-factor(data$subalgorithm,c("AG", "BTPa", "FTPa", "BTPr", "FTPr", "DiBS","Syll=Word","Utt=Word"))
...
pdf(paste(outfolder,"res.pdf"),width=8,height=5)
par(mar=c(3,3,0.5,0.5), xpd=TRUE)
plot(newdata$token_fscore~newdata$xval,col=newdata$col, pch="x",xaxt='n',xlab="", ylab='', yaxt='n', ylim=c(0,1), cex.lab = 1.3, cex.axis = 1, main="", cex.main=1.3)
...
mean_and_round<-function(x) round(mean(x,na.rm=T),2)
aggregate(parts$token_fscore,by=list(parts$subalgorithm,parts$language,parts$level),mean_and_round)->tab
...
write.table(mytab, paste0(outfolder,"meanscores.txt"), row.names=F, sep="\t&\t", eol="\t\\\\\n", quote = F )
...
table
table <-round(table,2)
write.table(table,paste0(outfolder,"anovascores.txt"), sep="\t&\t", eol="\t\\\\\n",quote = F)
```
...
mytab
write.table(mytab, paste0(outfolder,"anovaalgorithms.txt"), sep="\t&\t", eol="\t\\\\\n", quote = F )
...
wtt_LM=summary(lm(token_fscore~wtypes_wtokens_ratio*language*subalgorithm + (1 / file), subset=c(level=="words"), parts ))
wtt_LM
...
fit_statistics
write.table(fit_statistics, paste0(outfolder,"tab_fit_statistics.txt"), sep="\t&\t", eol="\t\\\\\n", quote = F )
```
...
scores_full<-cbind(wholes[wholes$language =='Chintang' & wholes$level =="words","token_fscore"], 
                       wholes[wholes$language =='Japanese' & wholes$level =="words","token_fscore"], 
...
scores_full
write.table(scores_full, paste0(outfolder,"scores_full.txt"),  sep="\t&\t", eol="\t\\\\\n", quote = F )
...
```{r fig:prerew}
library(ggplot2)
worddata <- parts[parts$level =='words',]
p <- ggplot(worddata, aes(token_precision, y=token_recall)) +  scale_shape_manual(values = c(4,7,16,8,15,3,17,5)) + ggtitle( 'Word Level, precision and recall')  + geom_point(aes(colour=language, shape=subalgorithm), size = 3) 
...
p <- ggplot(morphdata, aes(token_precision, y=token_recall)) +  scale_shape_manual(values = c(4,7,16,8,15,3,17,5)) + ggtitle( 'Morpheme Level, precision and recall')  + geom_point(aes(colour=language, shape=subalgorithm), size = 3) 
p<-p+ theme(panel.background = element_rect(fill = 'white', colour = 'black'))
p<-p+ scale_x_continuous(limits = c(0.2, 0.7))
p<-p+ scale_y_continuous(limits = c(0.2, 0.7))
p<- p + scale_color_hue(l=40, c=35)
...
chintang_only<-cbind(data[data$language =='Chintang' & data$level =="morphemes","token_fscore"], 
                      data[data$language =='Chintang' & data$level =="words","token_fscore"])
colnames(chintang_only)=c("morphemes", "words")
write.table(chintang_only, paste(outfolder,"chintang_only.txt"),  sep="\t&\t", eol="\t\\\\\n", quote = F )
